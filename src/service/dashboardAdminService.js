...
const dashboardOrder = async (page, limit) => {
    try {
        let offset = (page - 1) * limit;
        const { count, rows } = await db.Order.findAndCountAll({
            ...
            attributes: ['id', 'total_amount', 'customerName', 'phonenumber', 'address_detail', 'createdAt'],
            order: [['id', 'DESC']],
            include: [
                {
                    model: db.User,
...
const storeDashboardRevenue = async (page, limit) => {
    try {
        ...
        const countResult = await db.Order.findAll({
            ...
            attributes: [
                [Sequelize.fn('DISTINCT', Sequelize.fn('DATE', Sequelize.col('createdAt'))), 'distinct_dates']
            ],
        });
